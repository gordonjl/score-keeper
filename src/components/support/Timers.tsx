...
export function Timers() {
  const [timerType, setTimerType] = useState<'warm-up' | 'between-game' | 'injury' | null>(null)
      ...
      <div className="p-4 bg-base-200 rounded-box">
        {timerType ? (
            ...
            <div className="text-center">
              <div className="font-mono text-5xl flex justify-center">
                <span className="countdown">
                  <span style={{ '--value': Math.floor(time / 60) } as React.CSSProperties}></span>
                </span>
                :
                <span className="countdown">
                  <span style={{ '--value': time % 60 } as React.CSSProperties}></span>
                </span>
              </div>
              <div className="text-sm text-base-content/60 capitalize mt-2">{timerType} Timer</div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button className="btn btn-primary" onClick={() => startTimer('warm-up')}>Start Warm-up</button>
            <button className="btn btn-secondary" onClick={() => startTimer('between-game')}>Start Inter-Game</button>
            <button className="btn btn-accent col-span-1 sm:col-span-2" onClick={() => startTimer('injury')}>Start Injury Timeout</button>
          </div>
        )}
      </div>
    </div>
