...
const questions: Record<Exclude<State, Decision | 'START'>, { question: string; help: string }> = {
    COULD_REACH: {
        question: 'Could the obstructed player have reached the ball, made a good return, and were they making every effort to do so?',
...
export function LetStrokeDecision() {
  ...
  const renderCurrentState = () => {
    if (currentState === 'START') {
      return (
        <div>
          ...
          <p className="mb-4 text-base-content/80">Follow the questions to reach the correct decision based on the official rules.</p>
          <div className="divider"></div>
          <p className='font-bold text-lg'>Did interference occur?</p>
          <p className='text-sm text-base-content/60 mb-4'>Normally it is obvious whether interference has occurred. In a doubtful case, proceed with YES.</p>
          <div className="flex gap-4">
            <button className="btn btn-success flex-1" onClick={() => handleDecision('COULD_REACH')}>Yes</button>
            <button className="btn btn-error flex-1" onClick={() => handleDecision('NO_LET')}>No</button>
          </div>
        </div>
      )
    }
    ...
    if (Object.keys(decisionContent).includes(currentState)) {
        ...
        return (
            <div>
                <h3 className="text-xl font-semibold mb-2">Decision: <span className='text-primary'>{decision.title}</span></h3>
                <p className="mb-4 text-base-content/80">{decision.text}</p>
                <button className="btn btn-primary w-full" onClick={() => setCurrentState('START')}>Start Over</button>
            </div>
        )
    }
    ...
    return (
      <div>
        ...
        <div className="flex gap-4">
          <button className="btn btn-success flex-1" onClick={() => handleDecision(getNextState(currentState, true))}>Yes</button>
          <button className="btn btn-error flex-1" onClick={() => handleDecision(getNextState(currentState, false))}>No</button>
        </div>
        <button className="btn btn-ghost btn-sm mt-4" onClick={() => setCurrentState('START')}>Start Over</button>
      </div>
    )
  }
